...
"""
    ...
    render_kwargs = dict(
        ...
        formula=formula,
        params=table(
            df=(pd.read_csv(RESULTS_DIR / f"{prefix}/params.csv", index_col=0)),
    ...
    render_kwargs["predictions"] = image(
        f"{prefix}/counts.png", f"{prefix} predictions"
    )
...
def generate_readme():
    ...
    data_stats_pl = (
        ...
        .group_by(
            subset=(
                pl.when(pl.col("date") < VAL_START_DATE)
                .then(pl.lit("train"))
    ...
    loglik_mean = (
        ...
        .with_columns(
            subset=pl.when(pl.col("is_train"))
            .then(pl.lit("train"))
            .otherwise(pl.lit("validation"))
        )
        .drop("is_train")
    )
    loglik_diff = loglik_mean.with_columns(
    ...
    aic_train = loglik_with_n.select(
        ["subset"]
        + [
            (
    ...
    bic_train = loglik_with_n.select(
        ["subset"]
        + [
            (
                (
    ...
    rendered = template.render(
        ...
        data_stats=table(df=data_stats_pd, caption="Sample counts"),
        loglik_mean=table(
            df=(
